...
> 本文件是目录页，不是百科全书。渐进式披露：先读本文件 → 按需深入 `.agents/rules/` 与 `docs/`。
...
1. `pwd` 确认在仓库根
2. 完整阅读本文件
3. 读 `.agents/settings.json` 了解项目设置
4. 按需读规则：`.agents/rules/{coding,security,git,release}.md`
...
- **收拢约定**：agent 开发配置只放 `.agents/`；`skills/`（顶层）是 ganyu 运行时技能库，**不是开发辅助技能，勿混用**
- **安全底线**：详见 `.agents/rules/security.md`；绝不在代码/日志/提交中带出密钥
...
- [ ] 新增/修改逻辑有测试覆盖
- [ ] 提交信息遵循 `.agents/rules/git.md` 规范

## 升级处理

- 架构不明确 → 读 `docs/architecture.md` 或 ADR，仍不明则问
- 测试持续失败 → 记录进度与现象，标记需人工审查，不盲目绕过
- 涉及发布 → 遵循 `.agents/rules/release.md`（CI 全绿 → tag → release → 穷尽 review）
- 涉及安全面（密钥/沙箱/审计/执行面）→ 强制先读 `.agents/rules/security.md`
