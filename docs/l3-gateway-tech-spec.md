...
| 适配器 | 文件 | 传输 | 说明 |
|--------|------|------|------|
| `TelegramAdapter` | `gateway/telegram.rs` | 裸 `reqwest` 调 Bot API | `getUpdates` 长轮询（timeout=25s）+ `sendMessage`，零 SDK。`offset` 游标（`Mutex<i64>`）推进到 `update_id+1` 实现「不回放」：首轮取回待处理队列后即不再重放历史；`offset` 必须越过**所有** update（含无文本 / 非 `message` 键者），否则被跳过的 update 会重复取回。显式判别 `ok:false`（Telegram 错误返回 HTTP 200），并对非 2xx（如 5xx）退避 3s 自愈返回空（不向上抛错、不 panic，对齐 Discord/Slack）。`api` 字段（经 `with_base_url` + `no_proxy`）可注入回环 mock 服务器（`src/gateway/mock_http.rs`）做真实 HTTP 往返验证（见 `http_error_is_survivable_and_keeps_offset` 等）；v0.1.26 起网络层已全绿。 |
| `HttpBridge` | `gateway/http_bridge.rs` | axum 本地端点 `POST /message` | 请求/响应桥接：入队 → 挂起 oneshot（120s）→ `send()` 回送 HTTP 响应 |
| `DiscordAdapter` | `gateway/discord.rs` | 裸 `reqwest` 调 Discord REST API | `GET /channels/{id}/messages` 轮询（`before` 自最新向过去翻，越过游标即止）+ `POST` 发送，鉴权 `Bot <token>`；不回放历史（首次置位游标），跳过 bot/webhook 消息（`author.bot` / `webhook_id`）防自激循环，2s 空闲节流。分页用 `before` 而非 `after`：Discord `/messages` 固定「最新在前」，`after=<本页最小 id>` 会反复取回同一批消息，积压超一页时重复投递（v0.1.24 修复，见回归测试 `drains_backlog_larger_than_one_page_without_duplicates`）。`api_base` 抽为字段以便测试注入回环 mock 服务器（`src/gateway/mock_http.rs`）。 |
| `SlackAdapter` | `gateway/slack.rs` | 裸 `reqwest` 调 Slack Web API | `GET /conversations.history?channel=&oldest=` 轮询 + `POST /chat.postMessage`，鉴权 `Bearer <token>`；不回放历史（首次置位游标），分页跟随 `next_cursor` 直至 `has_more=false`，显式判别 `ok:false`（Slack 错误返回 HTTP 200），跳过 `bot_id` 消息防自激循环，`chat_id` 由查询频道注入（响应体不含该字段），2s 空闲节流。`api_base` 抽为字段以便测试注入回环 mock 服务器（`src/gateway/mock_http.rs`）；网络层经该 mock 服务器做真实 HTTP 往返验证（见 `ok_false_is_detected_and_survives` 等）。 |
...
> 已知取舍（两平台行为一致）：`SlackAdapter` 跳过带 `bot_id` 的消息、`DiscordAdapter`
> 跳过 `author.bot=true` 或带 `webhook_id` 的消息，以阻断「回复自己 → 再被读回 → 再回复」
> 的自激循环（两平台都会把本 bot 发出的消息一并返回在历史中）。Discord 侧过滤于 v0.1.23
> 补齐，至此两个平台的 bot 消息处理完全一致。
